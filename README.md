...
- **任务列表** — RecyclerView 展示所有任务，支持排序（开始时间/截止时间/优先级）
...
- **系统日历同步** — 同步到系统日历，到时系统通知提醒
  ...
  - 任务完成自动移除日历事件
  - 任务更新时同步更新日历事件
  - **MIUI 兼容**：优先使用系统已有日历，自动适配小米日历
  - **详细错误反馈**：同步失败时显示具体原因（权限/日历不存在/写入失败）

### 📎 任务详情与附件

- **说明备注** — 默认显示3行，支持多行文本、URL 链接
- **附件支持**
  - 从图库选取图片
  - 附件缩略图预览
  - **点击附件** — 调用系统图片查看器放大显示（使用 FileProvider）
  - **删除附件** — 附件下方显示红色删除按钮
...
| v3.1 | 修复点击任务项返回桌面的 Bug，升级 Java 11 |
| v3.2 | 进一步修复点击跳转详情页问题，添加异常保护，集成 Release 自动发布 |
| v3.3 | 修复主线程数据库访问异常 (allowMainThreadQueries)，修复状态 RadioGroup 重复选中 Bug |
| v3.4 | 说明备注默认5行，附件点击预览，附件删除功能 |
| v3.5 | 修复附件打开失败 (FileProvider 替代 Uri.fromFile) |
| v3.6 | 更新项目说明书 |
| v3.7 | 新增手动排序功能（时间/截止日期/优先级） |
| v3.8 | 说明备注默认3行 |
